...
dist/git_substitute_linux_386.tar.gz :
	stack build --docker --docker-auto-pull --install-ghc --os linux --arch i386 --local-bin-path ./bin --copy-bins
...
dist/git_substitute_linux_amd64.tar.gz :
	stack build --docker --docker-auto-pull --install-ghc --os linux --arch x86_64 --local-bin-path ./bin --copy-bins
...
dist/git_substitute_darwin_386.tar.gz :
	stack build --docker --docker-auto-pull --install-ghc --os osx --arch i386 --local-bin-path ./bin --copy-bins
...
dist: dist/git_substitute_linux_amd64.tar.gz dist/git_substitute_linux_386.tar.gz dist/git_substitute_darwin_amd64.tar.gz dist/git_substitute_darwin_386.tar.gz
